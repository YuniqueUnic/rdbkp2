...
- 备份命令会在调用 `docker stop` 之前检测容器状态，容器已停止时不会再出现无提示错误。
- 停止容器时会在超时时间内持续轮询状态，并在超时或失败时输出明确提示信息。
- 恢复流程会在解压前校验备份元数据并再次确认覆盖目标卷，防止误用的归档文件破坏现有数据。
- Docker 客户端调用升级到了最新的 `bollard::query_parameters` API，清除了编译期警告并与上游保持一致。
